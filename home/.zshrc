...
# Borrar Seleccion y activar BackSpace y Delete
delete-selected(){
...
zle -N delete-selected
bindkey '^?' delete-selected
bindkey '^H' delete-selected
####################################
# Activar el comportamiento de reemplazo al escribir
self-insert-overwrite-selection() {
        if (( REGION_ACTIVE )); then
                zle kill-region
        fi
        zle .self-insert
}
zle -N self-insert self-insert-overwrite-selection
